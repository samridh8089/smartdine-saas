...
function getPushLog(params) {
  const logStr = PropertiesService.getScriptProperties().getProperty('LAST_PUSH_LOG');
  if (logStr) {
    return { success: true, log: JSON.parse(logStr) };
  }
  return { success: false, message: 'No push log found' };
}